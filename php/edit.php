<?php
    ...
    if(isset($_POST["update"])){
        ...
        if($_FILES["picture"]["error"] == 0){
            ...
            $sql = "UPDATE `animals` SET `name`='$name', `address`='$address', `description`='$description', `age`='$age', `vaccinated`='$vaccinated', `breed`='$breed', `status`='$status',`picture`='{$picture[0]}', `agency_id_fk`= '$agencyID' WHERE id = $id";
        }else {
            $sql = "UPDATE `animals` SET `name` = '$name', `address`= '$address', `description`='$description', `age`='$age', `vaccinated`='$vaccinated', `breed`='$breed', `status`='$status', `agency_id_fk`= '$agencyID'  WHERE id = $id";
        }
           ...
           <div class="mb-4 mt-5">
            ...
            <div class="mb-4">
                ...
                <select class="form-select form-select mb-3" aria-label="breed" name="breed" id="breed" >
                    <option selected value="German Shepherd">German Shepherd</option>
